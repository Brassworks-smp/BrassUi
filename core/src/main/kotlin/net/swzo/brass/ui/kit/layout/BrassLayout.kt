...
import net.swzo.brass.ui.kit.layout.BrassLayout.spanningChildrenHeight
import net.swzo.brass.ui.kit.layout.BrassLayout.spanningChildrenWidth
...
object BrassLayout {
    ...
    /**
     * Height spanning the container's top to the bottom of its lowest child, plus [extra] trailing
     * room (the keycap bleed by default, so a bottom row's lip and hover lift stay scrollable).
     */
    ...
    /**
     * Width spanning the container's left to the right edge of its widest child, plus [extra] trailing
     * room (the keycap bleed by default, so a right-hand widget's outer ring is not clipped).
     */
    ...
    /**
     * Old name for [spanningChildrenHeight].
     *
     * Renamed because `contentHeight()` is *also* an instance method on `BrassFlow`,
     * `BrassMarkdown`, `BrassWrappedLabel` and `BrassGrid` - where it returns a measured `Float`, not
     * a constraint. Two things one call away from each other, with the same name and incompatible
     * types, is a coin flip every time you read either.
     */
    fun contentHeight(extra: Float = BrassWidget.BLEED_BOTTOM): HeightConstraint =
        spanningChildrenHeight(extra)

    /** Old name for [spanningChildrenWidth] - see [contentHeight]. */
    fun contentWidth(extra: Float = BrassWidget.BLEED_X): WidthConstraint =
        spanningChildrenWidth(extra)

    /**
     * A y just below [sibling], with [gap] between them - the common "flow after the previous block"
     * anchor, and correct even when [sibling] changes height (wrapped text growing a line, a
     * [BrassFlow] gaining a row).
     */
    fun below(sibling: UIComponent, gap: Float = 6f): YConstraint =
        basicYConstraint { sibling.getBottom() + gap }

    /**
     * A height equal to the **tallest** child, not the sum of them.
     *
     * Elementa's `ChildBasedSizeConstraint` adds its children's heights, which is correct for a stack
     * flowing one after another and wrong for children that overlap or are laid out in columns - two
     * columns of roster rows, a badge over a caption. Measured that way a card holding two five-row
     * columns comes out ten rows tall. This measures the lowest edge any child reaches instead, which
     * is the height the card actually needs, plus [extra] trailing room (the keycap bleed by default so
     * a bottom row's lip stays inside).
     *
     * It is the same measurement as [spanningChildrenHeight] - named for the *card sizing* case, where
     * "the tallest of several columns" is what the caller has in mind, so the call site reads as what
     * it means.
     */
