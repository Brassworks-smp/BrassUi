...
class BrassEased(
    initial: Float = 0f,
    /** Approach rate. Higher is snappier; ~15 is a brisk UI transition. */
    var speed: Float = 15f,
) {

    /** Where the value is heading. Assign it; the value follows. */
    ...
    /** True once the value has actually reached [target]. */
    ...
    /**
     * Advance one frame and return the new [value]. Call once per frame, before reading it.
     *
     * The step comes from [BrassClock] when something is driving it, so every animation on screen
     * moves by the same amount and a widget that skipped a few frames rejoins its neighbours instead
     * of catching up in one jump. Without a driver it falls back to timing itself - see [BrassClock].
     *
     * The clock is read before the settled check on purpose - see the class docs.
     */
    fun advance(): Float {
    ...
    /**
     * The fallback step for a toolkit nobody is driving.
     *
     * Read **unconditionally**, before any early return, which is the whole point of the class: the
     * hand-rolled version this replaced returned early when already settled *without* reading its
     * clock, so `lastNanos` held the moment the last animation ended and the next `dt` was however
     * many seconds the control had sat idle - one frame that covered the entire animation.
     */
    ...
    private companion object {
        /** Close enough to the target to land on it, rather than approaching forever. */
