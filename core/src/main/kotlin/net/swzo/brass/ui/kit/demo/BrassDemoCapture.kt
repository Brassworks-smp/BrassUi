...
interface BrassDemoCapture {
    ...
    /**
     * Write [image] as a PNG under [name], returning a human-readable path for the status line.
     *
     * PNG rather than anything cleverer because it is lossless — the toolkit is pixel art and a JPEG
     * would fuzz its 1-px borders — and because it is the format the wiki takes without fuss.
     */
    fun writePng(name: String, image: BufferedImage): String?

    /** Write [frames] as an animated GIF at [fps], returning a path for the status line. */
    fun writeGif(name: String, frames: List<BufferedImage>, fps: Int): String?

    /**
     * Release any resources [grab] acquired. Called when the browser closes; a no-op is fine.
     *
     * Reading the screen holds nothing, so a host doing that needs to do nothing here. It stays on the
     * interface so a host that *does* allocate — an offscreen target, a reusable buffer — has a place
     * to free it.
     */
    fun release() {}
    ...
    companion object {
        ...
        /** Whatever the host bound, or null off-game. */
        var current: BrassDemoCapture? = null
            private set

        /** Bind the host's implementation. Called once at startup. */
        fun bind(capture: BrassDemoCapture) {
            current = capture
        }

        /** Whether capturing is possible at all — the browser dims its shutter when this is false. */
